...
public class BurpExtender implements IBurpExtender,IProxyListener {
    ...
    public void registerExtenderCallbacks(IBurpExtenderCallbacks callbacks) {
        ...
        callbacks.setExtensionName("Domain Parser v2.0");
        callbacks.registerProxyListener(this);
        callbacks.printOutput("Success to extension load!");
        callbacks.printOutput("By default, this extension will query 300 records if the icp/ip result is parsed.");
    }
    ...
    public void processProxyMessage(boolean messageIsRequest, IInterceptedProxyMessage message) {
        if (!messageIsRequest) {
            ...
            String body = new String(
                    Arrays.copyOfRange(requestByte, iRequestInfo.getBodyOffset(), requestByte.length),
